...
class PIIDetector(Validator):
    ...
    Khi phát hiện PII → FailResult(fix_value=redacted_text)
    Với on_fail=OnFailAction.FIX, validated_output sẽ là text đã được redact.
    """
    ...
    def validate(self, value: str, metadata: dict):
        """
        Tìm PII trong value.
        Nếu phát hiện → FailResult(fix_value=redacted) để Guardrails thay thế output.
        Nếu sạch → PassResult().
        """
        redacted_text = value
        found_pii     = []
...
class JSONFormatter(Validator):
    ...
    Khi JSON cần sửa → FailResult(fix_value=repaired) để Guardrails thay thế output.
    Khi không thể sửa → FailResult(fix_value=fallback_json).
    Khi JSON hợp lệ → PassResult().
    """
    ...
    def validate(self, value: str, metadata: dict):
        ...
        Dùng FailResult + fix_value để Guardrails áp dụng output đã sửa.
        """
        # Thử parse trực tiếp — JSON hợp lệ, không cần sửa
        try:
            json.loads(value)
            return PassResult()
        ...
        try:
            ...
            print(f"  [FIXED] JSON repaired successfully")
            return FailResult(
                error_message="JSON was malformed and has been repaired",
                fix_value=json.dumps(parsed, indent=2, ensure_ascii=False),
            )
        except json.JSONDecodeError:
            pass

        # Không thể sửa — trả về JSON fallback
        fallback = json.dumps(
            {"error": "JSON khong hop le", "raw": value[:200]},
            ensure_ascii=False,
        )
        return FailResult(
            error_message="JSON is invalid and could not be repaired",
            fix_value=fallback,
        )
...
def demo_pii_guard():
    ...
    for label, text in test_cases:
        ...
        status = "CLEAN" if result.validation_passed else "REDACTED"
        print(f"\n[{label}] {status}")
        print(f"  Input:  {text}")
